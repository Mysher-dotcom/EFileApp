...
int _scannerIndex;
GetScannerInfoThread::GetScannerInfoThread(int devIndex,QObject *parent) : QObject(parent)
{
    _scannerIndex = devIndex;
    isStop = false;
}
...
/*
 * 通过camsdk获取设备信息，将信息发送给ScanManagerWindow主线程
 */
void GetScannerInfoThread::slotStartThread()
{
    ...
    {
        //无设备，发送信号给主线程，主线程绘制无设备的UI，并结束线程
        emit signalNoScanner();
        return;
    }
    else
    {
        ...
        for (int i = 0;i < nDevCount;i++)
        {
            ...
            QStringList devInfo;
            devInfo<<szDevType<<szNum<<"空闲"<<QString::number(i);
            devInfoMap.insert(1,devInfo);//参数加入集合
...
void GetScannerInfoThread::slotGetDevicePar()
{
   ...
   for(it = devParMap.begin();it!=devParMap.end();it++)
   {
    ...
    if(nResult == 0 && nCapCount > 0)
    {
        ...
        emit signalScannerParInfo(DataVar,"abc");//发送信号给主线程，主线程绘制设备信息和参数UI

...
public:
    explicit ScanWindow(QMap<int ,QString>,QWidget *parent = nullptr);
    ~ScanWindow();    
    ...
    void saveExit(bool isSaveAlll);//保存并退出
...
private slots:
    ...
    void imgListPressed(const QModelIndex index);//图像列表点击事件
    ...
    void slotScanSaveImage(char*,int,int,char*,int,int);//存图

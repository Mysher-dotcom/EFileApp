...
long CJpeg::saveImageToJpeg(uchar * src,int width,int height,int bpp,char * filePath,JPEGInfo jpegInfo)
{
    ...
    //上两句可以合并成：fstream fileout( "E://Colon.raw", std::ios::binary|std::ios::out);<br>　　 //或者 ofstream fileout("E://Colon1.raw", std::ios::binary|std::ios::out);
    fileout.write((char*)(desJpgBuf), exifBufflength + outputsize );
    fileout.close();
	delete desJpgBuf;
	return 0;
#else

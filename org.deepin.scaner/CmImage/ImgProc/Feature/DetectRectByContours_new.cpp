...
#include "../Enhancement/BrightnessBalance.h"
...
bool CDetectRectByContours_new::DetectRect_seg(Mat &src, Point* pt)
{
	...
	PerspectiveMatrix = findHomography(pts, Points1);//计算二维仿射变换矩阵
	warpPerspective(src, dst ,PerspectiveMatrix, Size(width,height));//透视变换
			...
			if (rectMaxArea>1000)
			{
				...
				if (rMaxRect.size.area()/(bw.cols*bw.rows)>0.95 || (PT4[0].x<2 && PT4[1].x>bw.cols-3) || (PT4[0].y<2 && PT4[3].y>bw.rows-3))
				{
					...
					if (count>0)
					{
						...
						//getRotationMatrix2D为图像旋转的函数，第一个参数center是旋转中心点，第二个参数angle为旋转角度

...
public:
	...
	// 返 回 值:	Mat	   处理后图像
	//************************************************************************ 
	static  Mat textEnhance(cv::Mat &src);
	...
	//返回类型：bool
	//************************************************************************  
	static Mat color_restoration(Mat &src);

	static vector<float> findThreshold(Mat src,int thresh, int type=0);
	...
	static int unevenLightCompensate1(Mat &image, int blockSize);//带插图文档优化

	static int unevenLightCompensate2(Mat &image, int blockSize);//红印文档优化
private:
	//************************************************************************  
	// 函数名称:    HomoFilter 
	// 函数说明:    同态滤波算法
	...
	static void mybilateralFilter(cv::Mat& src, cv::Mat& dst, cv::Size wsize, double spaceSigma, double colorSigma);
};
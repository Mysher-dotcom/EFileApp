...
//void rgb2lab( float R, float G, float B, float & l_s, float &a_s, float &b_s )
{
    ...
    if ( var_X > 0.008856 ) var_X = pow((double)var_X , ( 1./3. ) );
    else                    var_X = ( 7.787 * var_X ) + ( 16. / 116. );

    if ( var_Y > 0.008856 ) var_Y = pow((double)var_Y , ( 1./3. ));
    else                    var_Y = ( 7.787 * var_Y ) + ( 16. / 116. );

    if ( var_Z > 0.008856 ) var_Z = pow((double)var_Z , ( 1./3. ));
...
//void lab2rgb( float l_s, float a_s, float b_s, float& R, float& G, float& B )
{
    ...
    if ( var_R > 0.0031308 ) var_R = 1.055 * pow((double)var_R , ( 1 / 2.4 ))  - 0.055;
    else                     var_R = 12.92 * var_R;

    if ( var_G > 0.0031308 ) var_G = 1.055 * pow((double)var_G , ( 1 / 2.4 ) )  - 0.055;
    else                     var_G = 12.92 * var_G;

    if ( var_B > 0.0031308 ) var_B = 1.055 * pow( (double)var_B , ( 1 / 2.4 ) ) - 0.055;
...
void BGR2HSL(const uchar *bgr, float *hsl)
{
	...
	if ( delta == 0 ) {
		...
		if ( hsl[2] <= 0.5 )
			...
			hsl[1] = delta / (2.0 - (cmax + cmin));

		if (cmax == r1) {
			if ( g1 >= b1 )
				hsl[0] = int( 60.0 * (g1 - b1) / delta + 0.5);
			else
				hsl[0] = int( 60.0 * (g1 - b1) / delta + 360.0 + 0.5);
		} else if (cmax == g1) {
			hsl[0] = int( 60.0 * (b1 - r1) / delta + 120.0 + 0.5);
		} else { // cmax == b1
			hsl[0] = int( 60.0 * (r1 - g1) / delta + 240.0 + 0.5);
		}
	}
}
...
void BGR2HSB(const uchar *bgr, float *hsb)
{
	...
	hsb[2] = max / 255.0;
	hsb[1] = max == 0.0 ? 0.0 : (max - min) / (float) max;

	hsb[0] = 0.0;
	float fDecrease = 1.0;
	if (max == bgr[2] && bgr[1] >= bgr[0]) {
		hsb[0] = ( bgr[1] - bgr[0] ) * 60.0*fDecrease / (max - min) + 0.0*fDecrease;
	} else if (max == bgr[2] && bgr[1] < bgr[0]) {
		hsb[0] = (bgr[1] - bgr[0]) * 60.0*fDecrease / (max - min) + 360.0*fDecrease;
	} else if (max == bgr[1]) {
		hsb[0] = (bgr[0] - bgr[2]) * 60.0*fDecrease / (max - min) + 120.0*fDecrease;
	} else if (max == bgr[0]) {
		hsb[0] = (bgr[2] - bgr[1]) * 60.0*fDecrease / (max - min) + 240.0*fDecrease;
	}
}

		...
		int thickness, int lineType, int line_step, int blank_step )
{
	...
	len = sqrt((float) (p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y) );
...
int Curve::calcCurve(double *output_y)
{
	...
	vector<Point>::iterator start_point = points.begin();
	...
	int m = end_point->x - start_point->x+1;
...
void Curve::draw(Mat &mat,double z[256])
{
	int thinkness = 1;
	int n = 0;
	Point lastPoint;
...
void Curves::draw(Mat &mat,double z[256])
{
	
	if (CurrentChannel)  CurrentChannel->draw(mat,z);
}
...
void Curves::createColorTables(uchar colorTables[][256])
{
	...
	BlueChannel.calcCurve(z1);
	for (int i = 0; i < 256; ++i ) {
		colorTables[0][i] = z1[i];
	}

	GreenChannel.calcCurve(z1);
	for (int i = 0; i < 256; ++i )
		colorTables[1][i] = z1[i];

	RedChannel.calcCurve(z1);
	for (int i = 0; i < 256; ++i ) {
		colorTables[2][i] = z1[i];
	}

	uchar value;
	RGBChannel.calcCurve(z1);
	for (int i = 0; i < 256; ++i ) {
		for (int c = 0; c < 3; c++ ) {
			value = colorTables[c][i];
			colorTables[c][i] = z1[value];
		}
	}
}

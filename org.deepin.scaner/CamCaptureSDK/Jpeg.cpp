...
long CJpeg::saveImageFormJpegBuf(uchar * src,int width,int height,int bpp,uchar ** filePathBuf,JPEGInfo jpgInfo)
{
    ...
    uchar * jpgBuf = NULL;
    compresss_JPEG(jpgBuf,outputsize,src,width,height,bpp / 8,jpgInfo.compression);
    if (jpgBuf == NULL)
    {
        return -1;
    }
    ...
    desJpgBuf[0xd] = 1;
    if (jpgInfo.xResolution.b != 0 && jpgInfo.yResolution.b != 0)
    {
        long nXResolution = jpgInfo.xResolution.a / jpgInfo.xResolution.b;
        long nYResolution = jpgInfo.yResolution.a / jpgInfo.yResolution.b;
        WORDToString((uchar *)(desJpgBuf + 0xe),nXResolution,false);
        WORDToString((uchar *)(desJpgBuf + 0x10),nYResolution,false);
    }
    if(*filePathBuf==NULL)
    {
       *filePathBuf = new uchar[outputsize + exifBufflength];
        memcpy(*filePathBuf,desJpgBuf,exifBufflength + outputsize);
    }
    //memcpy()
    delete exifBuff;
    return true;

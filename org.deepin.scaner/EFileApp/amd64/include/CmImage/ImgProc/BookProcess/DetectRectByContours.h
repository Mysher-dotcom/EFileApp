...
public:
	...
	//返回值说明:
	//在图像上标识检测区域
	static bool DetectRect(IplImage *src, float & angle, CvPoint* pt = NULL, CvRect rect = cvRect(0,0,0,0));
	...
	//返回值说明:
	//裁切后图像
	static IplImage* DoDetectRect(IplImage *src, CvRect rect = cvRect(0,0,0,0));
	...
	//返回值说明:
	//裁切后图像
	static Mat DoDetectRect(Mat src, CvRect rect = cvRect(0,0,0,0));
...
private:
	...
	//内部-寻找目标轮廓
	static CvSeq* findObjectContour(IplImage *bw,float fAreaRatio,int nSizeTh,vector<float> vcRadioSizeTh,vector<CvPoint> vcBordePT,CvMemStorage *pStorage,CvRect crect=CvRect(0,0,0,0));
